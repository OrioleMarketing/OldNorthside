...
- [x] Audit the current `oldnorthsidebedandbreakfast.com` DNS zone and document the minimal web-routing changes required to bind the site without disrupting Resend or inbound email; retain Resend, MX, SPF, DKIM, and DMARC records while removing only the legacy root and `www` web A record during the custom-domain cutover.
...
- [x] Push the current verified Old Northside project to the authorized `OrioleMarketing/OldNorthside` GitHub repository and verify the remote revision.
